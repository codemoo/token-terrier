...
Token Terrier server-go 가 한 응답을 만들 때 **세 가지 종류**의 source 에서 데이터를 모은다.
...
- server-go 의 `claudeRefreshTask` / `codexRefreshTask` 가 **60 초 주기**로 호출
...
`JSONLPoller` 는 처음 보는 파일을 **EOF 에서 cursor 를 시작**한다. 즉 server-go 시작 시점 이전에 작성된 기존 라인은 무시. 이유:

- server-go 재시작마다 여러 백만 토큰의 historic 라인이 burn rate window 에 burst 로 들어가면 그래프가 비현실적으로 튐
- today total 도 server-go 재시작 후 0 부터 다시 누적 → 사용자 머릿속 "오늘 사용량" 과 안 맞을 수 있음. 향후 "자정부터 backfill" 을 옵션으로 추가할 수 있음
