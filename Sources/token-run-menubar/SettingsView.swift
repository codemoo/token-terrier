...
private struct ConnectionTab: View {
        ...
        Form {
            ...
            Section("Endpoint") {
                ...
                LabeledContent {
                    ...
                    Label("로컬", systemImage: "desktopcomputer")
                }
            }
    ...
    private func modeHint(_ mode: ConnectionMode, prefix: String) -> String {
        ...
        case .loopback:
            return "\(prefix) · 로컬 서버만 (127.0.0.1)"
        case .localDirect:
            return "\(prefix) · 로컬 OAuth 직접 read (서버 없음)"
        }
    }
}
    ...
    var body: some View {
        Form {
            Section {
                ...
                Text("로컬 서버가 만든 토큰 파일이 있으면 자동 입력됩니다. 다른 디바이스에서는 직접 붙여넣으세요.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
