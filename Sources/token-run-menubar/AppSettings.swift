...
public final class AppSettings {
    ...
    /// The local server writes `~/.config/token-usage/tokens.json` on first run.
    /// If we find that file, seed the bearer tokens automatically so the menu bar
    /// works out of the box on the same machine.
